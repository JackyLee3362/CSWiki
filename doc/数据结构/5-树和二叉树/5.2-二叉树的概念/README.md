...
- 完全二叉树 Complete Binary Tree
  ...
  - A. 若$i<[n/2]$ ，则结点 i 为分支结点，否则为叶子结点
  - B. 叶子结点只可能在层次最大的两层上出现。对于最大层中的叶子结点，都一次排列在该层最左边的位置上
  - C. 若有度为 1 的结点，则只可能有一个，且该结点只有左孩子而无右孩子（重要特征）
  - D. 按层序编号后，一旦出现某结点（编号为 i）为叶子结点或只有左孩子，则编号大于 i 的结点均为叶子结点
...
- 对完全二叉树按从上到下、从左到右的顺序依次编号 1，2，...，n，则有以下关系
  - A. 当 i >1 时，结点 i 的双亲的编号为 [ i/2 ]，即当 i 为偶数时，其双亲编号为 i/2，它是双亲的左孩子；当 i 为奇数时，其双亲的编号为 (i-1) / 2，它是双亲的右孩子
  - B. 当 2i ≤ n 时，结点 i 的左孩子编号为 2i，否则无左孩子
  - C. 当 2i + 1 ≤ n 时，结点 i 的右孩子编号为 2i，否则无右孩子
  - D. 结点 i 所在的层次（深度）为 $[ \log_2 i]+1$
